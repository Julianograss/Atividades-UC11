...
public class SistemaOperacional extends JFrame{
    public String[] usuarios = {"caixa1", "caixa2", "caixa3"};
    public String[] senhas = {"1", "2", "3"};
    ...
    public SistemaOperacional(){
        ...
        btnEntrar.addActionListener(e -> {
            ...
            if (usuario.equals("admin") && strSenha.equals("admin")){
                janelaGerente();
            } else if (usuario.equals(usuarios[0]) && strSenha.equals(senhas[0])){
                janelaOperador();
            } else if(usuario.equals(usuarios[1]) && strSenha.equals(senhas[1])){
                janelaOperador();
            } else if(usuario.equals(usuarios[2]) && strSenha.equals(senhas[2])){
                janelaOperador();
            }
            else {
                JOptionPane.showMessageDialog(null, "Usuario e senha não encontrados");
            }
        });
    ...
    public void janelaGerente(){
        ...
        JPanel pnlRelatorios = new JPanel(new BorderLayout());
        JPanel pnlRelatoriosInfos = new JPanel();
        pnlRelatoriosInfos.setPreferredSize(new Dimension(0, 50));
        JPanel pnlRelatoriosPDFS = new JPanel();
        pnlRelatoriosPDFS.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        JButton btnGeraPDF = new JButton("Gerar PDF de Vendas Mensal");
        btnGeraPDF.addActionListener(e -> {
            String[] col = {"Cód", "Descrição", "Qtd", "V.Unit", "Total"}; 

            DefaultTableModel model = new DefaultTableModel(col, 0);
            JTable tblItens = new JTable(model);
        ...
        pnlRelatoriosInfos.add(btnGeraPDF);
        JButton btnCSV = new JButton("Exportar Estoque (CSV)");
        btnCSV.addActionListener(e -> {
            exportarCSV();
        });
        pnlRelatoriosInfos.add(btnCSV);

        pnlRelatorios.add(pnlRelatoriosInfos, BorderLayout.NORTH);
        pnlRelatorios.add(pnlRelatoriosPDFS, BorderLayout.CENTER);
        
        JPanel pnlUsuarios= new JPanel();
        pnlUsuarios.setLayout(new BoxLayout(pnlUsuarios, BoxLayout.Y_AXIS));
        String[] colsUsuarios = {"Usuário", "Senha", "Cargo"};
        String cargo1 = "Operador de Caixa";
        DefaultTableModel model = new DefaultTableModel(colsUsuarios,0);
        JTable tblUsuarios = new JTable(model);
        JScrollPane scroll = new JScrollPane(tblUsuarios);
        scroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        model.addRow(new Object[]{"admin", "admin", "Gerente"});
        for(int i = 0; i < usuarios.length; i++){
            model.addRow(new Object[]{usuarios[i], senhas[i], cargo1});
        }
        pnlUsuarios.add(scroll);
